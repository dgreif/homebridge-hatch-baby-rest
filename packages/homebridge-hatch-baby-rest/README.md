...
The Hatch Baby (Rest Baby) exposes the routine-based on/off switch (first routine on / off), plus a Lightbulb service for brightness and color, and a Fan service for volume (rotation speed = volume %, so it's visible and adjustable in Apple's Home app). Audio track selection is not currently exposed.
