  ...
  private async connectToApiWithRetry() {
    ...
    for (;;) {
      try {
        ...
        // The rest client only retries network-level failures - an HTTP error
        // response at boot (e.g. a 429 from login, or a 5xx from the Hatch
        // API) lands here.  Without a retry, cached accessories would sit in
        // HomeKit responding to nothing until homebridge is restarted
        this.log.error('Error connecting to API')
